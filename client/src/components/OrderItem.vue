<template>
    <v-list-item :class="itemClass">
        <v-row class="d-flex align-center">
            ...
            <v-col cols="12" sm="12" :md="showColumn ? 2 : 3">
            ...
            <v-col v-if="showColumn" cols="12" sm="12" md="4" class="d-flex justify-center">
                <v-btn v-if="order.status === 'pending'" color="success" @click="orderSent(order.id)"
                    class="mr-2">Sent</v-btn>
                <v-btn v-if="order.status === 'shipped' && order.status !== 'cancelled'" color="primary"
                    @click="approveOrder(order.id)" class="mr-2">Complete</v-btn>
                <v-btn v-if="order.status === 'pending'" color="error" @click="cancelOrder(order.id)">Cancel</v-btn>
            </v-col>
        </v-row>
...
export default {
    ...
    computed: {
        ...
        showColumn() {
            return this.isAdmin && (this.order.status !== 'delivered' || this.order.status !== 'cancelled');
        }
    },
    methods: {
